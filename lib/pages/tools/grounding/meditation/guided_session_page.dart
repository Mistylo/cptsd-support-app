...
// Meditation sessions provide guided exercises.
// Users can customise session duration, background visuals and audio preferences.
// This prototype uses predefined local resources to demonstrate interaction desig
...
class _GuidedSessionPageState extends ConsumerState<GuidedSessionPage> {
  // Stores the current guided meditation session state, including the countdown timer and playback status
  ...
  // Stores the currently selected meditation background displayed during the session
  ...
  // Handles session timing, guided audio playback and completion behaviour
  ...
  void _playVoice() async {
    ...
    try {
      ...
      _voicePlayer.setSource(AssetSource(path)).then((_) => _voicePlayer.resume());
    }
  }
  ...
  // Allows the user to personalise the guided meditation session, including narration voice and background appearance
        ...
        ListTile(
          ...
          onTap: () {
            setState(() {
              ...
              if (_isRunning) {
                _voicePlayer.stop(); 
                // Plays the selected guided meditation recording.
          // The narration can be switched between the available voice options.
                _playVoice(); 
              }
            });
            setModalState(() {});
